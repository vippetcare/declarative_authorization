if Authorization.activate_authorization_rules_browser?
  ...
  class AuthorizationRulesController < ApplicationController
    ...
    def auth_to_dot(options = {})
      ...
      options[:filter_roles] ||= @users.collect(&:role_symbols).flatten.uniq if options[:only_relevant_roles] && @users
      ...
      engine.auth_rules.each do |auth_rule|
        ...
        auth_rule.contexts
                 .select { |c| options[:filter_contexts].nil? || c == options[:filter_contexts] }
                 .each do |context|
