...
class ChangeSupporterTest < Test::Unit::TestCase
  ...
  def test_prohibited_actions_role_to_any_user
    ...
    assert_not_equal 0, approaches.length
    assert !approaches.any? { |approach| approach.steps.any? { |step| step.class == Authorization::DevelopmentSupport::ChangeSupporter::AssignRoleToUserAction && step.role == :test_role } }
  end
